...
struct Graph
{
  ...
  void najudaljeniji_vrhovi(vertex* u) {
    ...
    while (!q.empty()) {
      ...
      q.pop();
      maxDist = std::max(maxDist, currentVertex->distance);

      for (edge *currEdge = currentVertex->edges; currEdge != NULL; currEdge = currEdge->next) {
        vertex *adjacent = currEdge->v;
        if (!adjacent || adjacent->color != WHITE) continue;

        adjacent->distance = currentVertex->distance + currEdge->weight;
