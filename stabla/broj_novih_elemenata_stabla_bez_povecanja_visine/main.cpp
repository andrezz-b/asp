...
struct Stablo {
  ...
  int max_height(Node *node, int height, int &num) {
    if (!node) return 0;
    num++;
    return std::max(max_height(node->left, 0, num), max_height(node->right, 0, num)) + 1;

  }

  void broj_cvorova() {
    int size = 0;
    // Maksimalan broj cvorova u stablu visine h = 2^(h + 1) - 1
    std::cout << pow(2, max_height(root, 0, size)) - size - 1 << endl;
  }
